...
install_service() {
    ...
    # 提示用户配置
    print_warning "请编辑配置文件: ${CONFIG_DIR}/config.example.js"
    print_warning "或者准备好远程配置文件URL"
    
    read -p "请输入配置文件路径或URL (回车跳过): " CONFIG_URL
...
show_status() {
    ...
    echo ""
    print_info "端口监听状态:"
    netstat -tlnp | grep ":443 " || print_warning "未检测到443端口监听"
